...
#endif
	{
		::fast_io::freestanding::nonoverlapped_bytes_copy_n(reinterpret_cast<std::byte const*>(__builtin_addressof(crc)),sizeof(std::uint_least32_t),ptr);
	}
}

inline constexpr std::uint_least32_t calculate_crc32_common(std::uint_least32_t crc,std::byte const* i,std::byte const* ed,std::uint_least32_t const* crctb) noexcept
{
	for(;i!=ed;++i)
	{
