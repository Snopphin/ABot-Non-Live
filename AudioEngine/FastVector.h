...
template <class T>
void fast_vector<T>::multiply(float value)
{
...
template <class T>
void fast_vector<T>::add(size_type index, const fast_vector& other)
{
    size_t NeededCapacity = index + other.m_capacity;
    ...
    for (size_type i = 0; i != other.m_size; i++)
    {
        ...
        {
            m_data[i + index] = abs(AudiodB) < abs(dB) ? std::clamp<int64_t>(other.m_data[i], INT16_MIN, INT16_MAX) : std::clamp<int64_t>(m_data[i + index], INT16_MIN, INT16_MAX);
        }
    }
}
